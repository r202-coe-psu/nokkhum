# Database Section
MONGODB_DB = "nokkhumdb"

APP_TITLE = "Nokkhum"

# compute config
NOKKHUM_COMPUTE_INTERFACE = "eth0"

# processor config
NOKKHUM_PROCESSOR_CMD = "nokkhum-processor"
NOKKHUM_PROCESSOR_RECORDER_PATH = "/tmp/nokkhum"
NOKKHUM_PROCESSOR_RECORDER_CACHE_PATH = "/tmp/nokkhum"
NOKKHUM_PROCESSOR_LOG_PATH = "/tmp/nokkhum"
NOKKHUM_PROCESSOR_ACQUISITOR_DEFAULT_FPS = 10
NOKKHUM_PROCESSOR_ACQUISITOR_DEFAULT_SIZE = (640, 480)
NOKKHUM_PROCESSOR_MOTION_DETECTOR_DURATION = 10
NOKKHUM_PROCESSOR_MOTION_DETECTOR_WAIT_MOTION_TIME = 10
NOKKHUM_PROCESSOR_RECORDER_CACHE_PATH_EXPIRED_DAYS = 3
NOKKHUM_PROCESSOR_RECORDER_LOGS_EXPIRED_DAYS = 7
# message config
NOKKHUM_MESSAGE_NATS_HOST = "localhost:4222"

# steaming config
NOKKHUM_STREAMING_URL = "http://localhost:8081"
NOKKHUM_WS_URL = "ws://localhost:8081"
# websocket config
# NOKKHUM_WS_URL = 'ws://localhost:8082'

# system login
NOKKHUM_LOGGIN_SYSTEMS = ["ENGPSU"]

# 0.00-24.00 format

DAIRY_TIME_TO_REMOVE = "1:0"
DUE_DATE = 1

NOKKHUM_STREAMING_MAX_WORKER = 5

TAR_TYPE = "xz"

NOKKHUM_COMPUTE_COMPRESSION_MAX_WORKER = 2
NOKKHUM_COMPUTE_CONVERTION_MAX_WORKER = 2

NOKKHUM_CONTROLLER_COMMAND_MAX_WORKER = 3
